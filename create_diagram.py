...
# --- Configure logging, BedrockProcessor, and DiagramGenerator ---
...
class BedrockProcessor:
    ...
    def get_architecture_json(self, text_input: str) -> dict:
        ...
        try:
            response = self.client.invoke_model(modelId=self.model_id, body=json.dumps(request_body), contentType='application/json', accept='application/json')
            ...
            raise Exception("Failed to invoke Bedrock model.") from e
        except Exception as e:
            logger.error(f"❌ [Bedrock] Failed to parse JSON from API response: {e}")
            raise Exception("Failed to get valid response from Bedrock API.") from e
...
class DiagramGenerator:
    ...
    def generate_xml_string(self, data: dict):
        ...
        tiers = {'user': 0, 'generic_client': 0, 'aws.network.route53': 1, 'aws.network.elb_application_load_balancer': 2, 'aws.compute.ec2_auto_scaling': 3, 'aws.database.rds_postgresql_instance': 4, 'aws.storage.s3': 4}
...
try:
    ...
    s3_client = boto3.client('s3') # Initialize S3 client once for warm starts
except Exception as e:
    logger.fatal(f"CRITICAL: Failed to initialize a processor or client: {e}")
    bedrock_processor = None
    diagram_generator = None

# --- Main Lambda Handler (with S3 save logic) ---
def lambda_handler(event, context):
    if not bedrock_processor or not diagram_generator:
    ...
    # --- CHANGE: Get bucket name from environment variables ---
    ...
    try:
        ...
        # --- CHANGE: Save to S3 instead of returning the file ---
        ...
        # --- CHANGE: Return a JSON response with the S3 location ---
        return {
            "statusCode": 200,
            "headers": {
        ...
        return {"statusCode": 500, "headers": {"Content-Type": "application/json"}, "body": json.dumps({"error": "An AWS service error occurred."})}
    except Exception as e:
